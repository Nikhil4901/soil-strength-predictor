...
LOG_FILE_PATH = os.path.join(BASE_DIR, 'predictions_log.csv')
...
if model_cbr and model_ucs and scaler:
    ...
    with st.form("dynamic_input_form"):
        ...
        with col2:
            ...
            if prediction_target == "Unconfined Compressive Strength (UCS)":
                ...
                cbr_input = 0.0  # Default value when not in UCS mode
        ...
        try:
            ...
            if prediction_target == "California Bearing Ratio (CBR)":
                ...
                st.session_state['last_prediction'] = {
                    ...
                    'MDD_g/cm³': mdd,
                    'Input_CBR_%': 'N/A',
                    'Predicted_Value': round(final_cbr, 2),
                    'Unit': '%'
                }
                ...
                st.session_state['last_prediction'] = {
                    ...
                    'OMC_%': omc,
                    'MDD_g/cm³': mdd,
                    'Input_CBR_%': cbr_input,
                    'Predicted_Value': round(final_ucs, 2),
                    'Unit': 'kg/cm²'
                }
    ...
    # Display download buttons if prediction exists
    if 'last_prediction' in st.session_state:
        st.divider()
        st.subheader("💾 Save & Download Results")
        
        # Create DataFrame for current prediction
        current_pred_df = pd.DataFrame([st.session_state['last_prediction']])
        
        # Convert to CSV for download
        csv_current = current_pred_df.to_csv(index=False)
        
        # Download button for current prediction
        st.download_button(
            label="📥 Download Current Prediction as CSV",
            data=csv_current,
            file_name=f"prediction_{st.session_state['last_prediction']['Prediction_Type']}_{datetime.now().strftime('%Y%m%d_%H%M%S')}.csv",
            mime="text/csv",
            help="Download the current prediction result as a CSV file"
        )
        ...
        try:
            ...
            st.success(f"✅ Prediction recorded in log file: `predictions_log.csv`")
            
            # Download button for all historical records
            if os.path.exists(LOG_FILE_PATH):
                log_df = pd.read_csv(LOG_FILE_PATH)
                csv_all = log_df.to_csv(index=False)
                
                st.download_button(
                    label="📊 Download All Historical Records as CSV",
                    data=csv_all,
                    file_name=f"all_predictions_log_{datetime.now().strftime('%Y%m%d_%H%M%S')}.csv",
                    mime="text/csv",
                    help="Download all historical prediction records"
                )
                
                # Display summary
                st.info(f"📈 Total predictions recorded: **{len(log_df)}**")
                
        except Exception as e:
            st.error(f"Error saving to log file: {e}")
